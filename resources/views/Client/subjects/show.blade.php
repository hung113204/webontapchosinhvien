...
@push('styles')
    <style>
        ...
        /* Đảm bảo căn lề của TinyMCE luôn hoạt động */
        .ck-content [style*="text-align: center"], .ck-content .text-center { text-align: center !important; }
        .ck-content [style*="text-align: right"], .ck-content .text-right { text-align: right !important; }
        .ck-content [style*="text-align: left"], .ck-content .text-left { text-align: left !important; }
        .ck-content [style*="text-align: justify"], .ck-content .text-justify { text-align: justify !important; }
        ...
        .ck-content img {
            ...
            margin-bottom: 1rem !important;
            display: inline-block; /* Để ảnh có thể nhận text-align: center từ thẻ P */
        }
        ...
        .ck-content pre {
            ...
            white-space: pre-wrap !important; /* ensure newlines are preserved */
        ...
        /* Định dạng bảng trong nội dung lý thuyết */
        .ck-content table {
            width: 100% !important;
...
@section('content')
    ...
    @php
        ...
        if ($monHoc->muc_do_mon_hoc == 2) {
            $levelText = 'Cơ sở ngành';
            $levelIcon = '📘';
        } elseif ($monHoc->muc_do_mon_hoc == 3) {
            $levelText = 'Chuyên ngành';
            $levelIcon = '⚡';
        } elseif ($monHoc->muc_do_mon_hoc == 4) {
            $levelText = 'Chuyên sâu';
            $levelIcon = '🔥';
        }
        ...
        <div class="container">
            <div class="subject-hero-inner">
                ...
                <div class="subject-hero-meta">
                    ...
                    @auth
                        @if ($tongBai > 0)
                            <div class="hero-meta-chip" id="hero-progress-chip"
                                style="background: rgba(16,185,129,0.15); border: 1px solid rgba(16,185,129,0.3); color: #10b981; gap: 8px;">
                                <i class="fas fa-chart-line"></i>
                                <span id="hero-progress-count">{{ $soBaiDaXong }}/{{ $tongBai }} bài</span> &nbsp;·&nbsp;
                                <strong id="hero-progress-percent">{{ $progressPercent }}%</strong>
                            </div>
                        @endif
                    @endauth
                </div>
                    ...
                    <div class="chapters-list">
                        @forelse ($monHoc->chuongHocs as $chuong)
                            @php
                                ...
                                // Kiểm tra xem chương này có chứa bài học đang mở hay không
                                $isActiveChapter = $activeBaiHoc && in_array($activeBaiHoc->id, $baiTrongChuong);
                            @endphp

                            <div class="chapter-item {{ $isActiveChapter || (!$activeBaiHoc && $loop->first) ? 'open' : '' }}">
                            ...
                            <div class="lesson-body">
                                ...
                                @if ($activeBaiHoc->cauHois && $activeBaiHoc->cauHois->count() > 0)
                                    ...
                                    <div class="quiz-block">
                                        ...
                                        @if ($quiz->giai_thich)
                                            <div id="quiz-explanation" style="display:none; margin-top: 16px;">
    ...
    @push('scripts')
        ...
        <script>
            window.SubjectConfig = {
                lessonId: {{ $activeBaiHoc->id ?? 0 }},
                routes: {
                    tiendoUpdate: "{{ route('client.tiendo.update') }}"
                },
                csrfToken: "{{ csrf_token() }}"
            };
        </script>
        <script src="{{ asset('frontend/asset/js/subject-show.js') }}"></script>
    @endpush
